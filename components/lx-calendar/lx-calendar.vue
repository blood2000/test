			...
			<swiper-item>
				...
				<view class="date_dl" v-show="!retract || index == to_prev_week_index" v-for="(item,index) in   week_list_prev_co" :key="index">
					<view class="dd" @click="item_click(vo,index,key)" v-for="(vo,key) in item" :key="key">
						...
						<view v-if="vo.dot" :class="vo.dot.dataTimeType === 0?'bg-white':vo.dot.dataTimeType === 1?'bg-blue':'bg-orange'" class="dot"></view>
					</view>
				</view>
				<view @click="open" class="flex align-center">
					<view class="xian"></view>
					<view class="retract icon"><text class="iconfont next icon-fanhui" :class="[retract ? '' : 'retract_icon']" /></view>
					<view class="xian"></view>
				</view>
			</swiper-item>
			<swiper-item>
		...
		doc_list_update(week_list = 'week_list'){
			...
			this[week_list].map((item,index)=>{
				list.push(item.map((vo,key)=>{
					const indexDate = this.dot_list.findIndex(res => res.dataTime === this.parseTime(vo.date, '{y}-{m}-{d}'));
					if(indexDate > -1){
						vo.dot = this.dot_list[indexDate];
...
.retract{
	display: flex; justify-content: center; align-items: center; height: 60rpx;;
	.iconfont{
