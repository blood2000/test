<template>
	<view class="xt__verify-code">
		...
		<!-- 输入框 - 组 :style="{ borderColor: code.length === index && cursorVisible ? boxActiveColor : boxNormalColor }" -->
		<view id="xt__input-ground" class="xt__input-ground">
			<template v-for="(item, index) in itemSize">
				<view
					:key="index"
					style="border: 0; border-radius: 10upx;"
					:style="{ background: boxNormalColor }"
